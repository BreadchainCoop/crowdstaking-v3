...
contract YieldDistributor is IYieldDistributor, OwnableUpgradeable {
    ...
    function getVotingPowerForPeriod(
        ...
        uint48 _latestKey = _currentCheckpoint._key < _start
            ? uint48(_start)
            : _currentCheckpoint._key;
        uint256 _totalVotingPower = _currentCheckpoint._value *
            (_end - _latestKey);
        ...
        for (uint32 i = _currentCheckpointIndex; i > 0; ) {
            ...
            if (_currentCheckpoint._key <= _start) {
                _totalVotingPower -=
                    _currentCheckpoint._value *
                    (_start - _currentCheckpoint._key);
                break;
            }
    ...
    {
        uint256 _available_yield = BREAD.balanceOf(address(this)) +
            BREAD.yieldAccrued();
        if (
            /// No votes were cast
            /// Already claimed this cycle
            currentVotes == 0 ||
            block.number < lastClaimedBlockNumber + cycleLength ||
    ...
    function distributeYield() public {
        ...
        for (uint256 i; i < projects.length; ++i) {
            uint256 _votedSplit = ((projectDistributions[i] *
                _votedYield *
                PRECISION) / currentVotes) / PRECISION;
    ...
    function _castVote(
        ...
        if (_points.length != projects.length)
        ...
        bool _hasVotedInCycle = accountLastVoted[_account] >
            lastClaimedBlockNumber;
        ...
        for (uint256 i; i < _points.length; ++i) {
            ...
            uint256 _currentProjectDistribution = ((_points[i] *
                _votingPower *
                PRECISION) / _totalPoints) / PRECISION;
     ...
     */
    function setMinRequiredVotingPower(
        uint256 _minRequiredVotingPower
    ) public onlyOwner {
     ...
     */
    function setyieldFixedSplitDivisor(
        uint256 _yieldFixedSplitDivisor
    ) public onlyOwner {
